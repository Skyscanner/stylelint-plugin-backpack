...
const { tokensByType } = require('../../utils/token-utils');
...
const COLORS = tokensByType('color')
  .filter((el) => el.deprecated !== true)
  .filter((el) => !el.name.includes('$bpk-color-'))
  .filter((el) => !el.name.includes('marcomms-'));
...
const rule = (primaryOption) => (postcssRoot, postcssResult) => {
  ...
  postcssRoot.walkDecls((decl) => {
    ...
    const expectedToken = _.find(COLORS, {
