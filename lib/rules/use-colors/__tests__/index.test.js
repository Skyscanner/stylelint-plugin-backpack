...
it('passes when setting color', async () => {
  const {
    ...
    code: 'a { color: $bpk-core-accent-day; font-size: 14rem }',
    config,
  });
...
it('passes when setting background-color', async () => {
  const {
    ...
    code: 'a { background-color: $bpk-surface-default-day; font-size: 14rem }',
    config,
  });
...
it('passes when using component color', async () => {
  const {
    ...
    code: 'a { color: $bpk-card-background-color }',
    config,
  });
...
it('gives an error and shows token to use', async () => {
  const {
    ...
    code: 'a { color: rgb(0, 98, 227); font-size: 14rem }',
    config,
  });

  expect(warnings).toHaveLength(1);
  expect(warnings[0].text).toBe(
    'Use the following Backpack token for color instead: $bpk-core-accent-day (backpack/use-colors)',
  );
});
